...
public class IAPManager : MonoBehaviour, IStoreListener
{
    ...
    public void BuyProduct(string productId, UnityAction<IAPOperationStatus, string, StoreProduct> OnCompleteMethod)
    {
        if (debug)
        {
            Debug.Log(this + "Buy Process Started for " + productId);
            ScreenWriter.Write(this + "Buy Process Started for " + productId);
        }
        ...
        for (int i = 0; i < shopProducts.Count; i++)
        {
            if (shopProducts[i].GetStoreID() == productId)
            {
                BuyProductID(shopProducts[i].GetStoreID());
            }
        }
            ...
            apple.RestoreTransactions((result, error) =>
             {
                ...
                else
                {
                    ...
                    {
                        //Restoration failed.
                        Debug.Log(this + "Restoration failed " + error);
                        ScreenWriter.Write(this + "Restoration failed " + error);
                    }
                }
            });
#else
    ...
    /// Get the price and currency code of the product as a string
    /// </summary>
    /// <param name="id">store product id</param>
    /// <returns>the localized price and currency of the product</returns>
    public string GetLocalizedPriceString(string id)
    {
        if (IsInitialized())
        {
            if (shopProducts != null)
            {
                var shopProduct = shopProducts.FirstOrDefault(x => x.GetStoreID() == id);
                if(shopProduct != null)
                    return shopProduct.localizedPriceString;
            }
            else
            {
                Debug.LogError("No products available -> Go to Window->Gley->Easy IAP and define your products");
    ...
    /// <param name="product">store product</param>
    /// <returns></returns>
    public string GetPrice(ShopProductNames product)
    {
        if (IsInitialized())
        {
    ...
    public void OnInitializeFailed(InitializationFailureReason error, string message)
    {
        if (debug)
        {
            Debug.Log(this + "OnInitializedFailed, error: " + error + " Message: " + message);
            ScreenWriter.Write(this + "OnInitializedFailed, error: " + error + " Message: " + message);
        }
        OnInitComplete(IAPOperationStatus.Fail, error.ToString(), null);
    }
